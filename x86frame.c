...
static F_accessList makeFormalAccessList(F_frame f, U_boolList formals)
{
	...
	for (int i = 0; escapes; escapes=escapes->tail,i++) {
...
F_frame F_newFrame(Temp_label name, U_boolList formals)
{
	...
	for (int i = 0; fmls; fmls=fmls->tail, i++) {
		T_stm s = NULL;
		T_exp dst = F_Exp(fmls->head, T_Temp(F_FP())),src = NULL;
		if (i < F_formalRegNum) {
			src = T_Temp(tl->head);
		} 
		else {
...
F_access F_allocLocal(F_frame f, bool escape)
{
	if (escape) {
		f->size += F_wordSize;
		F_access access = InFrame(-1 * f->size);
		return access;
	} 
	else {
...
AS_instrList F_procEntryExit2(AS_instrList body) {
	if (!returnSink) 
	    returnSink = Temp_TempList(F_SP(), Temp_TempList(F_FP(), F_calleeSavedReg()));// TODO:check later

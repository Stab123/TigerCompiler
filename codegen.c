...
static AS_instrList instrList=NULL, cur=NULL;

static void emit(AS_instr inst) {    //将后面要返回的指令登记在指令表中
...
static Temp_tempList munchArgs(int i, T_expList args, int *stack_size) {  //生成将所有参数传递到正确位置
    if (!args) return NULL;
        ...
        case T_MEM: {
            ...
            if (mem->kind == T_CONST) {
                ...
                if (mem->u.BINOP.op == T_plus) {
                    ...
                    if (mem->u.BINOP.right->kind == T_BINOP && mem->u.BINOP.right->u.BINOP.op == T_mul && 
                        mem->u.BINOP.right->u.BINOP.right->kind == T_CONST && mem->u.BINOP.right->u.BINOP.right->u.CONST == F_wordSize) {
                        //assert(mem->u.BINOP.right->u.BINOP.right->kind == T_CONST);
                        //assert(mem->u.BINOP.right->u.BINOP.right->u.CONST == 2);
                        Temp_temp b = munchExp(mem->u.BINOP.left);
...
static void muchMoveStm(T_stm s){
    char buf[256];
